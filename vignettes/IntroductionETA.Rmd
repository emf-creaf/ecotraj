...
Finally, one may calculate the temporal variation of states within each trajectory using `trajectoryInternalVariation()`:
```{r}
trajectoryInternalVariation(x)
```
The function returns the (absolute or relative) contribution of each observation to the temporal variation, the total sum of squares of temporal variation and an unbiased estimation of temporal variance. Note that in this example the third (more curved) trajectory has lower temporal variation, compared to the first and second (straighter) ones.
...
The asymmetric test is useful to determine if one trajectory is becoming closer to the other or if it is departing from the other. Finally, if the trajectories have been surveyed synchronously, one can perform a global test of convergence/divergence between trajectories, using `type = "multiple"` (in this example, this test cannot be run because the trajectories are not synchronous).
