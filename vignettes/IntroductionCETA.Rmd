...
names(fdtrajToy)
```
`d` is a distance matrix and `metadata` is a dataframe containing the information necessary to read it as a descriptor of **fixed-date trajectories**.
```{r}
...
```
The output gives the `cyclicalShifts()` computed in the same units as the input `time`, with positive values meaning advance and negative values meaning delay. The other columns give information on what exactly was compared. We will come back on this output in the real example below. 
...
### 3.2 Get a dissimilarity matrix and build cyclical trajectories
