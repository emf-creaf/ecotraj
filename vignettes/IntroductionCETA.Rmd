...
All this vocabulary helps CETA being more time-explicit than regular ETA. Let's build a toy dataset with **one cyclical trajectory** composed of **three cycles** to make this more visual:
```{r}
...
In CETA the "extract" functions take one or more **cyclical trajectories** and return a distance matrix `d` and associated descriptors allowing to isolate **cycles** or **fixed-date trajectories** (respectively functions `extractCycles()` and `extractFixedDateTrjaectories()`) for further analysis.  

#### 2.2.1 Fixed-date trajectories

Let's start with `extractFixedDateTrjaectories()` as it is the simplest one to handle:
```{r}
fdtrajToy <- extractFixedDateTrjaectories(d = dToy,
                                          sites = sitesToy,
                                          times = timesToy,
...
Let's look at the output given by `extractFixedDateTrjaectories()`:
```{r}
names(fdtrajToy)
```
...
```
The column `fdT` (for fixed-date trajectories) indicates to which fixed-date trajectories the different ecological states in `d` belong. The names in `fdT` are built by concatenating the site (especially useful if several cyclical trajectory are studied in parallel) with the string `fdT` and the name given as argument in `extractFixedDateTrjaectories` (if not provided, the date is used as default).

Using a combination a the new distance matrix `d` and it's descriptors in `metadata`, the output of `extractFixedDateTrjaectories` can be fed in other ETA functions to study fixed-date trajectories. The key is to use the values of `fdT` column as substitute for `sites`. For instance we can visualize the fixed-date trajectories as follows:
```{r fig = TRUE, fig.height=5, fig.width=5, fig.align = "center"}
...
Ok let's now look at how extraction works for **cycles**. The function `extractCycles()` works very similarly to `extractFixedDateTrjaectories()`. Few differences are that: it does not allows to specify names for cycles (they are by default the concatenation of site names and "C1", "C2" etc...); it allows to change the `startdate` of the cycles (for instance allowing to flexibly choose when a cycle start: *e.g. an annual cycle does not always need to start in January*).
```{r}
cycleToy <- extractCycles(d = dToy,
                          sites = sitesToy,
                          times = timesToy,
                          cycleDuration = cycleDurationToy)
```

Just like `extractFixedDateTrjaectories()`, `extractCycles()` has outputs `d` and `metadata`:
```{r}
names(cycleToy)
```

When inspecting `metadata`, we find the column `Cycles` <!-- MCA: I would use lowercase for consistency -->, containing the unique names of all cycles, analogously to column `fdT` in the output of `extractFixedDateTrjaectories()`. In addition and importantly, `extractCycles()` returns the column named `IntExt` <!-- MCA: this could be a logical vector indicating external states, instead -->, which has no analog in `extractFixedDateTrjaectories()`:
```{r}
head(cycleToy$metadata)
```

The `IntExt` column gives an information on *internal vs external* ecological states. This is an important distinction that solve what we call the **"December-to-January segment problem"** and that ‒ *unfortunately* ‒ complicates a bit the use of the outputs of `extractCycles()`. We explain this in the following section.
...
This duplication allows to easily, and correctly, compute most of the ETA metrics for cycles such as, for instance, distances. Akin to what we did with fixed-date trajectories, the key is to use the column `Cycles` as input for `sites` in the ETA function :
```{r}
...
Another metric that need special handling in CETA is cycle smoothness, which is estimated using function `cycleConvexity()`. This metric is the cyclical equivalent of directionality for non-cyclical trajectories. Smoothness is related to convexity. A cycle with high smoothness is interpreted as simpler (more convex) than a cycle with low smoothness. Low smoothness is due to concavities or "bends" in more than two dimension indicative of a more complex (or nosier) cyclical dynamic.  
The function `cycleConvexity()` does not uses the outputs of `extractCycles()`. This is because the original full trajectory is needed for computation. Instead, `cycleConvexity()` take the same inputs as `extractCycles()`:
```{r}
cycleConvexity(d = dToy,
               sites = sitesToy,
               times = timesToy,
               cycleDuration = cycleDurationToy)
```
...
![The CETA functions (blue) allow to format data to describe fixed-date trajectories or cycles. The reformated data (Output of functions `extractFixedDateTrjaectories()` and `extractCycles()`) can then be used as inputs for other ETA functions (orange) to obtain the metrics of interest. However, when woking on cycles, keep in mind that external ecological states must be treated with care for some applications and that some metrics (obtained with functions `cycleConvexity` and `cycleShift()`) are obtained directly from the original data, not from the output of `extractCycles()`.](CETA_workflow.png){width="100%"}
...
```{r}
cyclesNSZoo <- extractCycles(d = northseaZoo$Dist,
                             sites = northseaZoo$sites,
...
However, remember that cycle smoothness is not obtain using the output of `extractCycles`, instead we get it using the same inputs as for `extractCycles` such as:
```{r}
cyclesZooSmooth <- cycleConvexity(d = northseaZoo$Dist,
                                  sites = northseaZoo$sites,
                                  times = northseaZoo$times,
                                  cycleDuration = 1,
                                  minEcolStates = 12)
...
Let's now look at the fixed-date trajectories. First prepare the data using `extractFixedDateTrjaectories`:
```{r}
fdtrajNSZoo <- extractFixedDateTrjaectories(d = northseaZoo$Dist,
                                            sites = northseaZoo$sites,
                                            times = northseaZoo$times,
                                            cycleDuration = 1,
                                            namesFixedDate =
...
As we did for cycles, we can compute shape metrics on fixed-date trajectories (length and directionality):
```{r}
...
```{r}
#descriptive stats (MAYBE THAT COULD BE DONE AUTOMATICALLY BY extractFixedDateTrjaectories???):
