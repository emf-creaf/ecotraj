...
b. A *character vector* specifying the site (i.e. sampling unit) corresponding to each ecosystem state; 
c. An *integer vector* specifying the survey (i.e. census) corresponding to the sampling of each ecosystem state. 
d. A *numeric vector* specifying the survey time corresponding to each ecosystem state.

In ETA, sampling units do not need to be surveyed synchronously nor the same number of times. 

However, in this example we assume all three sites have been surveyed synchronously four times. 
...
### 2.4 Trajectory lengths, speeds, angles and directionality
...
trajectoryLengths(x)
```
When survey times are available, it may be of interest to calculate segment or trajectory speeds. One can obtain the speed of trajectory segments and the total pathway speed using function `trajectorySpeeds()`:
...
### 2.5 Relative positions within trajectories
...
### 2.7 Distances between segments and trajectories
...
### 3.1 Displaying trajectories in PCoA
