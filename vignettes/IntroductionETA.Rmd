...
In ETA, sampling units do not need to be surveyed synchronously nor the same number of times. However, in this example we assume all three sites have been surveyed synchronously four times. 
...
Note that `surveys` may be omitted, and in this case site surveys are assumed to be in order. The function returns an object of class `trajectories` that contains all the information for analysis:
...
Functions `trajectoryPCoA()`and `trajectoryPlot()` can be used to display a subset of trajectories if we apply function `subsetTrajectories()`:
...
We start by defining our trajectories, which implies combining the information about distances, sites and surveys:
