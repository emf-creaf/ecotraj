...
Although trajectories in this data set are not particularly noisy, package `ecotraj` incldues a function that allows smoothing out trajectory noise. This is done by applying a smoothing kernel over each trajectory, which turns consecutive ecosystem states more similar. Function `trajectorySmoothing()` performs the smoothing operation and returns a modified distance matrix:
