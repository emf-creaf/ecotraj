...
*Ecological Trajectory Analysis* (ETA) is a framework to analyze the dynamics of ecological entities (e.g. communities or ecosystems). The key aspect of ETA is that dynamics are represented using trajectories in a chosen multivariate space. These trajectories are then analyzed and compared geometrically. 
...
To specify dynamics of a set of target entities, the following data items need to be distinguished: 
  ...
  a. A set of ecological states (i.e. coordinates in a space $\Omega$) implicitly described using a **distance matrix** $d$.
...
When estimating dissimilarities between a set of trajectories one is building a new space. One can display it in two dimensions using mMDS.
```{r, echo=T, fig=TRUE, fig.height=5, fig.width=5, fig.align = "center"}
...
We start by defining our trajectories, which implies combining the information about distances, entities (sites) and surveys (observation times are omitted):
...
In this case we obtain that tau is decreasing and the test is significant, which indicates that forest structures are overall converging.
...
```{r trajectory_8, echo=T, fig.height=4, fig.width=8, fig.align = "center"}
oldpar <- par(mfrow=c(1,2))
...
Apparently, it would be the bimodal distribution of tree sizes, with a large proportion of saplings, what determines the distinctiveness of plot '8' from the remaining.
