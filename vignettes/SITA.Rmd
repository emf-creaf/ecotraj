...
This vignette present some ecological applications used by Sturbois at al. (under review) in the new stable isotope trajectory analysis framework. Example datasets have been included in the package for reproducibility. After the calculation of the necessary distance- and direction-based metrics, the vignette focuses, for each ecological applications, on the creation of trajectory charts.
...
This is a subset of the data sets provided in: 

+ Kernaléguen, L., Arnould, J.P.Y., Guinet, C., Cherel, Y., 2015.  Determinants of individual foraging specialization in large marine vertebrates, 
the Antarctic and subantarctic fur seals. *Journal of Animal Ecology* 1081–1091.
...
In this section, we illustrate how to calculate trajectory metrics to characterize the foraging strategy of each fur seal. First, we calculate net changes relative to the initial state (i.e. the distance between SI state of each whisker section and the initial SI state):
```{r, echo=T}
...
We then calculate trajectory segment lengths, i.e. the distance between consecutive  whisker sections in the SI space:
```{r, echo=T}
...
Finally, we determine the angle (α) of consecutive trajectory segments with respect to the second axis of the 2D diagram:
```{r, echo=T}
...
Here we aim to define groups of fur seals depending on the similarity of their foraging strategy. We need first to calculate distances between pairs of complete trajectories in the SI space:
```{r, echo=T}
...
Here we display trophic trajectories of all individuals, in plots corresponding to combinations of species and gender. To facilitate such plots, we creation of a vector with the combination of species and gender:
```{r, echo=T}
furseals$sp_gender<-paste(furseals$Sexe, furseals$Species, sep=" ")
```

We now create the plot of trajectories in the SI space. Arrows connects all whiskers section stable isotope values from t1 to t30 (i.e. most recent to oldest SI values). Colors corresponds to trajectory clusters and shape to breeding sites.
```{r, echo=TRUE, fig.height=4, fig.width=6}
...
Preparation of the data to compute the trajectory diagram
```{r}
NC<-Net_changes[,-30]
...
Transformation of the data set into long format
```{r}
...
Arrows connects all whiskers section stable isotope values from t1 to t30 (i.e. most recent to oldest stable isotope values). Colours corresponds to trajectory clusters.
...
Preparation of the data set to compute the trajectoy rose
```{r}
Angl<-Angles
colnames(Angl)<-2:30
...
Angles α were calculated in 2D Ωδ space (δ 13C/δ 15N) and represented by range (15◦) of direction. Bars size represent the number of trajectory segments (all individual within each trajectory clusters). 
```{r, fig.height=4, fig.width=6}
...
Trajectory metrics:
```{r}
Net_changes<-trajectoryLengths2D(pike[,8:9],pike$ID,pike$Time, relativeToInitial=TRUE) 
...
Identification of trajectory clusters:
```{r, fig.height=4, fig.width=6}
Hst=3
colstd<-c("black","yellow","green","blue","grey","red")
hsxy <- hclust(Ds, "ward.D2")
...
Preparation of the data set to compute trajectory diagrams and density curves
```{r}
...
Arrow represent trajectory path for each pit-tagged individual. Colors correspond to trajectory clusters. The dashed line separates piscivorous from zooplanktivorous individuals [zooplanktivorous (δ15N < 10) vs piscivorous (δ15N > 10)]. Density curves represents the distribution of all samples according to δ13C (x) and δ15N (y), and capture (green=release; red=departure).
...
## 5. Spatio-temporal variability of δ13C and δ15N modelled isoscapes in the Northeast Pacific


The data sets used in this application come from: 
...
Loading isoscape data
```{r load Alaska, echo=T}
data("isoscape")
```
...
angles transformation for the use with geom_spoke
```{r, echo=T}
angle<-data$Angles
Angles2<-c()
...
SITA metrics were mapped to illustrate stable isotope spatio-temporal dynamics. Direction of arrows (angle α) illustrate direction in the modelled 2D Ωδ space according to increase and/or decrease in δ13C and δ15N values (0-90°: + δ13C and + δ15N; 90-180°: + δ13C and - δ15N; 90-180°: + δ13C and - δ15N; 180-270°: + δ13C and - δ15N). Length of arrows and colored background rasters illustrate modelled trajectory segment length at each station.
```{r, fig.height=4, fig.width=6}
ggplot(data, 
          aes(x = Longitude, 
              y = Latitude, 
              fill = Lengths, 
              angle = Angles2, 
              radius = scales::rescale(Lengths, c(0.3, 1)))) +
