...
legend(x="topleft",col=c("blue","red"),pch=15,unique(northseaZoo$sites))
```
It is still a very busy graph but note that some data points are not there anymore (such as the outlier from SNS far on PCoA axis 2). This is because we asked èxtractCycles()` to only take complete years (`minEcolStates = 12`). We can nonetheless see that more recent years (clearer lines) seem to be moving towards negative values of PCoA axis 2 suggesting a shift in zooplankton community composition across the whole time series and in both parts of the North Sea.
...
StatCyclesZoo <- data.frame(sitesCycles,yearCycles,cyclesZooLengths$Trajectory,cyclesZooConv)
...
plot(SNScycles$yearCycles,SNScycles$cyclesZooLengths.Trajectory,type="l",las=1,ylab="Cycle lengths",xlab="Years",ylim=c(2.5,5),col="red")
...
```{r fig = TRUE, fig.height=5, fig.width=7, fig.align = "center"}
#First figure: Lengths
plot(SNSfdT$monthFDT,SNSfdT$fdtrajZooLengths.Trajectory,type="l",las=1,ylab="Fixed-date trajectory lengths",xlab="Month",ylim=c(8,25),col="red")
