...
First, we calculate net changes relative to the initial state (i.e. the distance between SI state of each whisker section and the initial SI state):
```{r, echo=T}
...
We now create a diagram to display fur seal trophic trajectories in the SI space. Panels correspond to the combination of species and gender. In each panel, X-Y axes are defined by SI values. Arrows connects all whiskers section SI states from t1 to t30 (i.e. most recent to oldest SI state). Colors corresponds to trajectory clusters and shape to breeding sites.
