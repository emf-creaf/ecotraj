...
<!-- MCA: the commented code lines are not needed in my computer. If you call data from you system, the vignette will not be reproducible-->
```{r}
library(ecotraj)
# library(devtools)
...
Let's build a toy dataset with **one cyclical trajectory** composed of **three cycles**:
```{r}
...
We can visualize the cyclical trajectory using the function `trajectoryPCoA`, as we would do for any trajectory within ETA.
```{r fig = TRUE, fig.height=5, fig.width=5, fig.align = "center"}
trajectoryPCoA(d = dToy, 
               sites = sitesToy,
               surveys = timesToy, 
               lwd = 2,length = 0.2)
```

We see that the cyclical trajectory is composed of three cycles that are not yet explicitly identified. Similarly, the fixed-dates trajectories are not isolated. This is the goal of the "build" functions in CETA.
<!-- MCA: I wonder whether it is better to call them extractCycles() and extractFixedDateTrajectories(). Additionally, I also wonder whether it would be good to have a extractSiteTrajectory() function, subsetting the trajectory of one site, starting from data containing many trajectories. This could have the same output structure (distance matrix and metadata). -->
...
names(fdtrajToy)
```
`d` is a modified distance matrix and `metadata` is a dataframe containing the information necessary to read it as a descriptor of **fixed-date trajectories**.
```{r}
head(fdtrajToy$metadata)
```
The column `fdT` (for fixed-dates trajectories) indicates to which fixed-dates trajectories the different ecological states in `d` belong. The names in `fdT` are built by concatenating the site (especially useful if several cyclical trajectory are studied in parallel) with the string `fdT` and the name given as argument in `fdtrajBuild` (if not provided, the date is used as default).

Using a combination a the new distance matrix `d` and it's descriptors in `metadata`, the output of `fdtrajBuild` can be fed in other ETA functions to study fixed-date trajectories. The key is to use the values of `fdT` column as substitute for `sites`. For instance we can visualize the fixed-date trajectories as follows:
```{r fig = TRUE, fig.height=5, fig.width=5, fig.align = "center"}
...
Note that we find, as expected 10 fixed-dates trajectories (one for each date of each cycle). They are all linear and pretty much parallel, which is what we expect given the trend that we put in our toy dataset.

We can also, compute the directionality of the fixed-date trajectories using:
```{r}
...
When inspecting `metadata`, we find the column `Cycles` <!-- MCA: I would use lowercase for consistency -->, containing the unique names of all cycles, analogously to column `fdT` in the output of `fdtrajBuild()`. In addition and importantly, `cycleBuild()` returns the column named `IntExt` <!-- MCA: this could be a logical vector indicating external states, instead -->, which has no analog in `fdtrajBuild()`:
```{r}
head(cycleToy$metadata)
```

The `IntExt` column gives an information on *internal vs external* ecological states. This is an important distinction that solve what we call the **"December-to-January segment problem"** and that ‒ *unfortunately* ‒ complicates a bit the use of the outputs of `cycleBuild()`. We explain this in the following section.
...
<!-- according to the above definitions, PCoA would exclude external segments. Another thing is the arrow representation -->
* Computation of cycle smoothness or cyclical shifts, where external segments are required.

<!-- * Computation of Bdiv for cycles. MCA: I think that a general function for ETA would be more appropriate -->
...
We will detail how to treat external ecological states within CETA in the next sections, but first, let's explore the outputs of `cycleBuild()` to make all this more palatable:
```{r}
...
Note that, after the centering the location of duplicated sites is no longer the same. The distances between cycles after centering are:
...
The distinction between internal and external ecological states also complicates graphic representation as we want to compute a weighted PCoA taking in account the internal vs external nature of ecological states. <!-- MCA: See my comment above. I think that if trajectories have not been centered, PCoA should use only internal states, whereas if they have been centered, then external states are different because their positions will differ for the two cycles. Perhaps there should be a boolean parameter to include external states in the PCoA representation. If not included, the segments that the two cycles have in common could be drawn using dashed arrows. We should perhaps discuss this. -->
A special function is thus available within CETA for this purpose **(-ish, I'm not super sure of the function)**.
...
<!-- MCA: I wonder why not calling it cycleConvexity() -->
Another metric that need special handling in CETA is cycle smoothness, which is estimated using function `cycleSmoothness()`. This metric is the cyclical equivalent of directionality for non-cyclical trajectories. Smoothness is related to convexity. A cycle with high smoothness is interpreted as simpler (more convex) than a cycle with low smoothness. Low smoothness is due to concavities or "bends" in more than two dimension indicative of a more complex (or nosier) cyclical dynamic.  
...
A last novelty of CETA is to provide a way to generalize the concept of phenological advances and delays for multidimensional data such as those used in community ecology. The function that computes cyclical shifts is `cycleShift()`. By default, it will estimate all the shifts that are possible to compute on a given cyclical trajectory. The computation relies on geometrical projection and comparison of a ecological states of interest to a reference cycle. The computation time can be long for large datasets. The function takes the same inputs as `cycleBuild()`:
```{r}
cycleShift(d = dToy,
           sites = sitesToy,
           times = timesToy,
           cycleDuration = cycleDurationToy)
```
The output gives the `cyclicalShift()` computed in the same units as the input `time`, with positive values meaning advance and negative values meaning delay. The other columns give information on what exactly was compared. We will come back on this output in the real example below. 
