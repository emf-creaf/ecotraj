...
```{r}
max(as.vector(Dcent) - as.vector(dist(xy_cent)))
```
...
```{r}
Dcent_excluded <- centerTrajectories(D, sites, 
                                     exclude = excluded)
```

We can see the effect of the new centering using:
```{r, echo=T, fig=TRUE, fig.height=4, fig.width=5, fig.align = "center"}
trajectoryPCoA(Dcent_excluded, sites, traj.colors = c("black","red", "blue"), lwd = 2,
               survey.labels = T)
```
...
Distances in `avoca_D_man` are calculated using the Manhattan metric. As before we use `trajectoryPCoA` to display the relations between forest plot states in this space and to draw the (uncentered) trajectory of each plot:
...
```{r, echo=T, fig=TRUE, fig.height=4, fig.width=5, fig.align = "center"}
avoca_D_man_cent_initial <- centerTrajectories(avoca_D_man, avoca_sites, 
                                               exclude = all_but_first) 
avoca_D_man_cent_final <- centerTrajectories(avoca_D_man, avoca_sites, 
...
trajectoryPCoA(avoca_D_man_cent_initial,  avoca_sites, avoca_surveys,
               traj.colors = RColorBrewer::brewer.pal(8,"Accent"), 
               axes=c(1,2), length=0.1, lwd=2)
title("Reference: initial state")
trajectoryPCoA(avoca_D_man_cent_final,  avoca_sites, avoca_surveys,
               traj.colors = RColorBrewer::brewer.pal(8,"Accent"), 
               axes=c(1,2), length=0.1, lwd=2)
...
```{r trajectory_smooth}
avoca_D_man_smooth <- smoothTrajectories(avoca_D_man, avoca_sites, avoca_surveys)
```
